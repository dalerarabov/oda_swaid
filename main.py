...
def fetch_data(
...
) -> Tuple[List[Dict], datetime, Optional[datetime]]:
    """
    ...
    logger.info("[%s] Запрос: %s", device_name, params)
    try:
...
def format_table(
    ...
    headers = ["Time", "Device", "HR", "LF/HF", "RMSSD", "SDRR", "SI"]
    table = []

    table.append([
        f"Start: {start_time.strftime('%Y-%m-%d %H:%M:%S') if start_time else '-'}",
        "", "", "", "", "", ""
    ])
    ...
    table.append([
        f"End: {end_time.strftime('%Y-%m-%d %H:%M:%S') if end_time else '-'}",
        "", "", "", "", "", ""
    ])

    current_time = datetime.now(MY_TZ).strftime("%Y-%m-%d %H:%M:%S")
    table.append([f"Now: {current_time}", "", "", "", "", "", ""])

    return tabulate(table, headers=headers, tablefmt="grid", maxcolwidths=[20, 20, 10, 10, 10, 10, 10])
...
def fetch_and_process_data(
    ...
    for future in as_completed(futures):
        ...
        try:
            ...
            logger.error("[%s] Ошибка обработки: %s", device_name, error)
